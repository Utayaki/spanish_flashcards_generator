...
class WordBankDatabase:
    ...
    def __init__(
        ...
        *,
        ...
        self.schema_path = (
            Path(schema_path) if schema_path else Path(__file__).with_name("schema.sql")
        )
        self._drill_sync_enabled = True
    ...
    def create_noun_lexical_item(
        ...
        *,
        ...
        self._sync_drill_cards(lexical_item_id)
    ...
    def _sync_drill_cards(self, lexical_item_id: int) -> None:
        if not self._drill_sync_enabled:
            return
        try:
            from drill.database import open_default_drill_database
            from drill.sync import sync_drill_cards_for_lexical_item

            drill_db = open_default_drill_database(initialize=True)
            sync_drill_cards_for_lexical_item(self, drill_db, lexical_item_id)
        except Exception:
            return


SpanishLexicalItemDatabase = WordBankDatabase
