...
CREATE TABLE IF NOT EXISTS words (
    ...
    word_type TEXT NOT NULL CHECK (
        word_type IN ('noun', 'verb', 'adjective', 'determiner', 'other')
    ),
...
CREATE TABLE IF NOT EXISTS nominal_details (
    ...
    gender_availability TEXT NOT NULL CHECK (
        gender_availability IN ('masc', 'fem', 'both', 'ambiguous')
    ),
...
CREATE TABLE IF NOT EXISTS nominal_inflections (
    ...
    form TEXT CHECK (
        form IS NULL OR length(trim(form)) > 0
    ),
...
CREATE TABLE IF NOT EXISTS verb_participles (
    ...
    participle_type TEXT NOT NULL CHECK (
        participle_type IN ('present', 'past')
    ),
    ...
    is_irregular INTEGER NOT NULL DEFAULT 0 CHECK (
        is_irregular IN (0, 1)
    ),

    PRIMARY KEY (word_id, participle_type),
...
FOR EACH ROW
WHEN (SELECT word_type FROM words WHERE id = NEW.word_id)
     NOT IN ('noun', 'adjective', 'determiner')
BEGIN
    SELECT RAISE(
        ABORT,
        'nominal_details can only be used for noun, adjective, or determiner words'
    );
END;
...
BEGIN
    SELECT RAISE(
        ABORT,
        'nominal_inflections can only be used for noun, adjective, or determiner words'
    );
END;
...
BEFORE INSERT ON nominal_inflections
FOR EACH ROW
WHEN NEW.form IS NOT NULL
 AND (
    (
        (SELECT gender_availability FROM nominal_details WHERE word_id = NEW.word_id)
        = 'masc'
        AND NEW.gender = 'fem'
    )
    OR
...
BEGIN
    SELECT RAISE(
        ABORT,
        'form is not allowed for this gender availability'
    );
END;
...
BEGIN
    SELECT RAISE(
        ABORT,
        'other_details can only be used for other words'
    );
END;
...
BEGIN
    SELECT RAISE(
        ABORT,
        'verb_participles can only be used for verb words'
    );
END;
...
BEGIN
    SELECT RAISE(
        ABORT,
        'verb_forms can only be used for verb words'
    );
END;
